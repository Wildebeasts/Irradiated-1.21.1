...
@EventBusSubscriber
public class ZombieRadAttack {
    
    @SubscribeEvent
    public static void onZombieAttack(LivingDamageEvent.Pre event) {
        ...
        if (attacker instanceof Zombie && victim instanceof Player player) {
            float radiationAmount = calculateRadiation(player);
            
            // Apply radiation multiplier from config
            radiationAmount *= RadiationConfig.ZOMBIE_RADIATION_MULTIPLIER.get().floatValue();
    ...
    private static float calculateRadiation(Player player) {
        // Base radiation from zombie attack
        float baseRadiation = 2.0f;
